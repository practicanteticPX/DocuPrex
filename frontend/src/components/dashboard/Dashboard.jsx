...
function Dashboard({ user, onLogout }) {
  ...
  // Estados para dashboard de sesiones activas (Admin only)
  ...
  useEffect(() => {
    ...
    newSocket.on('session:closed', (data) => {
      console.log('🚨 Sesión cerrada remotamente por admin:', data);
      ...
      console.log(`🔍 Comparando: Usuario actual ID=${currentUser.id} vs Sesión cerrada userId=${data.userId}`);

      // Si el userId del evento coincide con el usuario actual, forzar logout INMEDIATO
      if (data.userId === currentUser.id) {
        console.warn(`⚠️ TU SESIÓN (ID=${currentUser.id}) HA SIDO CERRADA POR UN ADMINISTRADOR - Logout silencioso`);

        // Limpiar localStorage INMEDIATAMENTE
        localStorage.removeItem('token');
        localStorage.removeItem('user');

        // Logout silencioso - SIN alert
        onLogout();
      } else {
        console.log(`✅ Sesión cerrada de otro usuario (ID=${data.userId}), no afecta a este usuario (ID=${currentUser.id})`);
      }
    });
  ...
  // Cargar documentos retenidos al montar o cambiar de tab
  useEffect(() => {
  ...
  const loadRetainedDocuments = async () => {
    ...
    try {
      ...
      if (!isAuthError(err)) {
        setError('Error al cargar documentos retenidos');
      }
    } finally {
  ...
  return (
    <>
              ...
              {showSettings && (
                <>
                  ...
                  >
                    ...
                    {/* Botón de Sesiones Activas (Solo para e.zuluaga) */}
                    {(user?.email === 'e.zuluaga@prexxa.com.co' || user?.email === 'e.zuluaga') && (
                      <button
                        onClick={() => {
                      ...
                      >
                        <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ width: '16px', height: '16px', color: '#3b82f6' }}>
                ...
                <div className="my-docs-filters">
                  <div className="filter-search">
                    ...
                    <input
                      ...
                      className="filter-search-input"
                      placeholder="Buscar por nombre de documento..."
                      value={retainedDocsSearchTerm}
                      onChange={(e) => setRetainedDocsSearchTerm(e.target.value)}
                    />
                    {retainedDocsSearchTerm && (
                      <button
                        className="clear-search-btn"
                        onClick={() => setRetainedDocsSearchTerm('')}
                        title="Limpiar búsqueda"
                      >
                        <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                          <path d="M18 6L6 18M6 6L18 18" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
              ...
              return (
                <div key={sig.id} className="signers-dropdown-item" title={tooltipText}>
                  ...
                  <div className="signer-info">
                    <div className="signer-name">{displayName}</div>
      ...
      {/* Dashboard de sesiones activas (Admin only) */}
      <SessionsDashboard
